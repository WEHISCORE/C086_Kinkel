...
- **Directional gene sets** (i.e. logFC available from previous data). 
  - `Male: Het vs. Del`: DEGs from a comparison of male Smchd1-het and Smchd1-del B cells from aged mice (>12 months old)^[Sarah "used SeqMonk/EdgeR to compare Smchd1-del to Smchd1-het, keeping males and females separate" (email from Sarah).]
  - `Female: Het vs. Del`: DEGs from a comparison of female Smchd1-het and Smchd1-del B cells from aged mice (>12 months old)^[Sarah "used SeqMonk/EdgeR to compare Smchd1-del to Smchd1-het, keeping males and females separate" (email from Sarah).]. 
  ...
  - `Quiescence Group Venezia (2004)`: Genes that have their maximum expression at non-cycling times (**T**ime **O**f **M**ax = TOM = 0, 1, 10, and 30 days post-treatment) in a time-course experiment designed to stimulate adult HSCs to proliferate [@venezia2004molecular].
    - The p-value (ANOVA) is based on about 1500 genes that changed significantly over the time course while the logFC is a separate comparison between FL HSCs (naturally proliferative) and adult HSCs (naturally quiescent).
  - `Smchd1 female B cells (chrX)`: logFCs for X chromosome genes from a comparison of Smchd1-del versus Smchd1-het B cells from female mice
    - Sarah performed this analysis in SeqMonk and exported the complete DE results as a CSV file.
    - These data are from the same experiment as the `Female: Het vs. Del` gene list.

```{r}
# NOTE: The `as.data.frame()` calls are needed because read_excel() returns a 
#       tibble but `limma::fry()` does not support tibbles.
library(readxl)
male_het_vs_del <- read_excel(
  here("data", "sarahs_gene_lists", "MaleHetvDel_EdgeR.xlsx"))
male_het_vs_del <- as.data.frame(male_het_vs_del)
female_het_vs_del <- read_excel(
...
y_venezia_proliferation <- venezia_proliferation[
  , c("Gene.Symbol", "Log2.Fold.Change..FL.HSC.vs.Adult.HSC..")]
y_venezia_proliferation <- y_venezia_proliferation[
  !duplicated(y_venezia_proliferation$`Gene.Symbol`), ]
y_venezia_quiescence <- venezia_quiescence[
  , c("Gene.Symbol", "Log2.Fold.Change..FL.HSC.vs.Adult.HSC..")]
y_venezia_quiescence <- y_venezia_quiescence[
  !duplicated(y_venezia_quiescence$`Gene.Symbol`), ]
# TODO: Clarify how this gene set is to be used.
# y_smchd1_femBcells <- smchd1_femBcells[
#   smchd1_femBcells$`FDR (EdgeR stats p<1.0 after correction)` < 0.05, 
#   c("Feature", "Log2 Fold Change (EdgeR stats p<1.0 after correction)")]
# y_smchd1_femBcells <- y_smchd1_femBcells[
#   !duplicated(y_smchd1_femBcells$Feature), ]
y_smchd1_femBcells <- smchd1_femBcells[
    # smchd1_femBcells$Chromosome == "X", 
  smchd1_femBcells$Chromosome == "X" &
    smchd1_femBcells$`FDR (EdgeR stats p<1.0 after correction)` < 0.05,
  c("Feature", "Log2 Fold Change (EdgeR stats p<1.0 after correction)")]
y_smchd1_femBcells <- y_smchd1_femBcells[
...
y_index <- list(
  ...
  `Smchd1 female B cells (chrX)` = y_smchd1_femBcells)
```
...
ym_venezia_proliferation <- venezia_proliferation[
  , c("Gene.Symbol", "Log2.Fold.Change..FL.HSC.vs.Adult.HSC..")]
ym_venezia_proliferation <- ym_venezia_proliferation[
  !duplicated(ym_venezia_proliferation$`Gene.Symbol`), ]
ym_venezia_quiescence <- venezia_quiescence[
  , c("Gene.Symbol", "Log2.Fold.Change..FL.HSC.vs.Adult.HSC..")]
ym_venezia_quiescence <- ym_venezia_quiescence[
  !duplicated(ym_venezia_quiescence$`Gene.Symbol`), ]
# TODO: Clarify how this gene set is to be used.
# ym_smchd1_femBcells <- smchd1_femBcells[
#   smchd1_femBcells$`FDR (EdgeR stats p<1.0 after correction)` < 0.05, 
#   c("Feature", "Log2 Fold Change (EdgeR stats p<1.0 after correction)")]
# ym_smchd1_femBcells <- ym_smchd1_femBcells[
#   !duplicated(ym_smchd1_femBcells$Feature), ]
ym_smchd1_femBcells <- smchd1_femBcells[
    # smchd1_femBcells$Chromosome == "X", 
  smchd1_femBcells$Chromosome == "X" & 
    smchd1_femBcells$`FDR (EdgeR stats p<1.0 after correction)` < 0.05,
...
ym_index <- list(
  ...
  `Smchd1 female B cells (chrX)` = ym_smchd1_femBcells)
```
...
yf_venezia_proliferation <- venezia_proliferation[
  , c("Gene.Symbol", "Log2.Fold.Change..FL.HSC.vs.Adult.HSC..")]
yf_venezia_proliferation <- yf_venezia_proliferation[
  !duplicated(yf_venezia_proliferation$`Gene.Symbol`), ]
yf_venezia_quiescence <- venezia_quiescence[
  , c("Gene.Symbol", "Log2.Fold.Change..FL.HSC.vs.Adult.HSC..")]
yf_venezia_quiescence <- yf_venezia_quiescence[
  !duplicated(yf_venezia_quiescence$`Gene.Symbol`), ]
# TODO: Clarify how this gene set is to be used.
# yf_smchd1_femBcells <- smchd1_femBcells[
#   smchd1_femBcells$`FDR (EdgeR stats p<1.0 after correction)` < 0.05, 
#   c("Feature", "Log2 Fold Change (EdgeR stats p<1.0 after correction)")]
# yf_smchd1_femBcells <- yf_smchd1_femBcells[
#   !duplicated(yf_smchd1_femBcells$Feature), ]
yf_smchd1_femBcells <- smchd1_femBcells[
    # smchd1_femBcells$Chromosome == "X", 
  smchd1_femBcells$Chromosome == "X" & 
    smchd1_femBcells$`FDR (EdgeR stats p<1.0 after correction)` < 0.05,
...
yf_index <- list(
  ...
  `Smchd1 female B cell (chrX)` = yf_smchd1_femBcells)
```
