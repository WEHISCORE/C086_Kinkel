...
description: "UMI-based analysis"
author:
  - name: Peter Hickey
...
- This report analyses the UMI counts
...
```{r}
x <- DGEList(
  counts = as.matrix(assay(sce, "UMI_counts")),
  samples = colData(sce),
...
```{r}
x_keep_exprs <- filterByExpr(x, group = x$samples$group, min.count = 5)
x <- x[x_keep_exprs, , keep.lib.sizes = FALSE]
table(x_keep_exprs)
```

```{r}
y_keep_exprs <- filterByExpr(y, group = ym$samples$group, min.count = 5)
...
<aside>
The need for further normalization and the rationale for choosing quantile normalization is more relevant for the analysis of the UMI counts than the read counts.
</aside>
...
```{r}
# TODO: Is there a more general list of imprinted genes?
...
- Fit a model with a term for `smchd1_genotype_updated` and `sex`.
...
- An interactive `r BiocStyle::Biocpkg("Glimma")` MD plot of the differential expression results is available from [`output/DEGs/UMIs/glimma-plots/individual_tech_reps.md-plot.html`](../output/DEGs/UMIs/glimma-plots/individual_tech_reps.md-plot.html).
...
- CSV file of results available from [`output/DEGs/UMIs/individual_tech_reps.DEGs.csv.gz`](../output/DEGs/UMIs/individual_tech_reps.DEGs.csv.gz).
...
We saw earlier that the consensus correlation between technical replicates, as estimated by `limma::duplicateCorrelation()`, is very close to zero (`x_fit$correlation` = `r scales::number(x_fit$correlation, accuracy = 0.001)`).
This suggests that the technical replicates are not very consistent.
However, the 'consensus correlation' has a somewhat technical meaning and a value close to zero does not simply mean that the technical replicates are uncorrelated.
Indeed, shown in [`output/figures/logCPM_pairs_plot.UMIs.pdf`](../output/figures/logCPM_pairs_plot.UMIs.pdf) are scatterplots of the log-CPM for pairs of technical replicates from each mouse which shows the replicates are highly correlated.
...
pdf(here("output", "figures", "logCPM_pairs_plot.UMIs.pdf"))
for (l in levels(x_v$targets$genotype.mouse)) {
  j <- x_v$targets$genotype.mouse == l
  E <- x_v$E[, j]
  pairs(E, upper.panel = panel.cor, main = l)
}
dev.off()
```
...
- An interactive `r BiocStyle::Biocpkg("Glimma")` MD plot of the differential expression results is available from [`output/DEGs/UMIs/glimma-plots/aggregated_tech_reps.md-plot.html`](../output/DEGs/UMIs/glimma-plots/aggregated_tech_reps.md-plot.html).
...
- CSV file of results available from [`output/DEGs/UMIs/aggregated_tech_reps.DEGs.csv.gz`](../output/DEGs/UMIs/aggregated_tech_reps.DEGs.csv.gz).
...
- Fit a model with a term for `smchd1_genotype_updated`.
...
- An interactive `r BiocStyle::Biocpkg("Glimma")` MD plot of the differential expression results is available from [`output/DEGs/UMIs/glimma-plots/aggregated_tech_reps.males.md-plot.html`](../output/DEGs/UMIs/glimma-plots/aggregated_tech_reps.males.md-plot.html).
...
- CSV file of results available from [`output/DEGs/UMIs/aggregated_tech_reps.males.DEGs.csv.gz`](../output/DEGs/UMIs/aggregated_tech_reps.males.DEGs.csv.gz).
...
- An interactive `r BiocStyle::Biocpkg("Glimma")` MD plot of the differential expression results is available from [`output/DEGs/UMIs/glimma-plots/aggregated_tech_reps.females.md-plot.html`](../output/DEGs/UMIs/glimma-plots/aggregated_tech_reps.females.md-plot.html).
...
- CSV file of results available from [`output/DEGs/UMIs/aggregated_tech_reps.females.DEGs.csv.gz`](../output/DEGs/UMIs/aggregated_tech_reps.females.DEGs.csv.gz).
